 ...
 *
 */
public class IeConcat8bit implements IeConcat {
...
//> CONSTRUCTOR
	 ...
	 */
	private IeConcat8bit(int reference, int partSequence, int totalParts) {
		if(reference != (reference & 0xFF)) {
...
//> ACCESSORS
	...
	public int getLength() {
		return TpduUtils.TP_UDH_IEI_CONCAT_SMS_8BIT_LENGTH;
	}
	/** @see UserDataHeaderPart#getIEId() */
	public int getIEId() {
		return TpduUtils.TP_UDH_IEI_CONCAT_SMS_8BIT;
	}
...
//> STATIC FACTORY METHODS
	 ...
	 * @param in
	 * @return a new instance of {@link IeConcat8bit}
	 * @throws IOException
	 */
	public static IeConcat8bit getFromStream(PduInputStream in) throws IOException {
		int reference = in.read();
		int totalParts = in.read();
		int partSequence = in.read();
		
		return new IeConcat8bit(reference, partSequence, totalParts);
	}
