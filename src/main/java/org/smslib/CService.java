...
public class CService {
	...
	/**
	 * number of times to retry an AT command before giving up when the response
	 * is a CMS error
	 */
	...
	/**
	 * The SMSC number for this device. If set <code>null</code>, a blank value
	 * will be used. This usually allows a device to determine the SMSC number
	 * for itself.
	 */
	private String smscNumber;
	/**
	 * The PIN number to set for this device, or <code>null</code> if none has
	 * been supplied.
	 */
	...
	/**
	 * if <code>true</code>, {@link #connect()} will throw an exception if asked
	 * for SIM PIN2 and {@link #simPin2} is null.
	 */
	 ...
	 *            The manufacturer of the modem (i.e. Wavecom, Nokia, Siemens,
	 *            etc).
	 ...
	 *            TODO
	 */
	public CService(String port, int baud, String gsmDeviceManufacturer,
			String gsmDeviceModel, String catHandlerAlias) {
		smscNumber = "";
	 ...
	 * <p>
	 * <strong>Warning</strong>: The method return the "theoretical" status of
	 * the connection, without testing the actual connection at the time of the
	 * call.
	 * 
	...
	/**
	 * Sets the SMSC number. Needed in rare cases - normally, you should
	 * <strong>not</strong> set the SMSC number yourself and let the GSM device
	 * get it from its SIM.
	 * 
	 * @param smscNumber
	...
	/**
	 * Sets the SIM Pin 2. Some GSM modems may require this to unlock full
	 * functionality.
	 * 
	 * @param simPin
	 *            The SIM PIN #2 code.
	...
	/**
	 * Sets the message handler for ASYNC mode. The handler is called
	 * automatically from SMSLib when a message is received. This handler is
	 * valid only for Asynchronous operation mode - in other modes, it is not
	 * used.
	 * 
	 * @param messageHandler
	 *            The message handler routine - must comply with
	 *            ISmsMessageListener interface.
	...
	/**
	 * Sets the call handler. Works in ALL modes. The handler is called
	 * automatically once SMSLib receives an incoming call.
	 * 
	 * @param callHandler
	 *            The call handler routine - must comply with ICallListener
	 *            interface.
	...
	/**
	 * Sets the Async Poll Interval (in seconds) - is every how many seconds
	 * will SMSLib poll the GSM modem for new messages.
	 * 
	 * @param secs
	...
	/**
	 * Sets the Keep-Alive Interval - every how many seconds the Keep-Alive
	 * thread will run and send a dummy OK command to the GSM modem. This is
	 * used to keep the serial port alive and prevent it from timing out. The
	 * interval is, by default, set to 30 seconds which should be enough for all
	 * modems.
	 * 
	 * @param secs
	...
	/**
	 * Sets the number of retries that SMSLib performs during dispatch of a
	 * message, if it fails to get a response from the modem within the timeout
	 * period.
	 * <p>
	 * After the number of retries complete and the message is not sent, SMSLib
	 * treats it as undeliverable.
	 * <p>
	...
	/**
	 * This method only appears to be used in
	 * {@link ATHandler#sendMessage(int, String, String, String)}
	 * 
	 * @return number of times to retry an AT command before giving up when the
	 *         response is a CMS error
	 */
	 ...
	 * <p>
	 * The default protocol is PDU. If you want to change it, you must call this
	 * method after constructing the CService object and before connecting.
	 ...
	 * <p>
	 * Normally, SMSLib tries to read the available storage locations reported
	 * by the modem itself. Sometimes, the modem does not report all storage
	 * locations, so you can use this method to define yours, without messing
	 * with main SMSLib code.
	 * 
	 * @param loc
	 *            The storage locations (i.e. a string similar to "SMME", which
	 *            is specific to each modem)
	 */
	 ...
	 * <p>
	 * The connect() function should be called before any operations. Its
	 * purpose is to open the serial link, check for modem existence, initialize
	 * modem, start background threads and prepare for subsequent operations.
	 * 
	 * @throws SMSLibDeviceException
	 *             If there was a problem initiating the connection to the
	 *             device
	 * @throws IOException
	 ...
	 * @see #disconnect()
	 */
	// TODO this now throws a lot more exceptions! The JAVADOC needs updating to
	// reflect this. Also, it would be wise
	// to check that throwing this many different exceptions from one method is
	// actually reasonable behaviour - It seems
	// logical, but also looks ridiculous.
	public void connect() throws SMSLibDeviceException, IOException,
			TooManyListenersException, PortInUseException, NoSuchPortException,
			DebugException, UnsupportedCommOperationException {
		synchronized (_SYNC_) {
			if (isConnected())
				...
				try {
					...
					if (atHandler.isAlive()) {
						...
						if (atHandler.isWaitingForPin(pinResponse)) {
							...
							if (!atHandler.enterPin(getSimPin()))
								throw new InvalidPinException();

							pinResponse = atHandler.getPinResponse();
						}

						// If we're still waiting for the PIN, we made need the
						// 2nd PIN number
						...
						if (atHandler.isWaitingForPuk(pinResponse)) {
							// TODO is there a more suitable exception? If not,
							// one should be created, as we ultimately want to
							// catch this and allow the user to enter the PUK
							// FIXME or is it PUK2?
						...
						if (atHandler.getStorageLocations().length() == 0)
							atHandler.initStorageLocations();
						log.info("MEM: Storage Locations Found: "
								+ atHandler.getStorageLocations());
						switch (protocol) {
						case PDU:
							log.info("PROT: Using PDU protocol.");
							if (!atHandler.setPduMode())
								throw new NoPduSupportException();
							break;
						case TEXT:
							log.info("PROT: Using TEXT protocol.");
							if (!atHandler.setTextMode())
								throw new NoTextSupportException();
							break;
						default:
							throw new UnrecognizedHandlerProtocolException(
									protocol);
						}
	 ...
	 * <p>
	 * This should be the last function called. Closes serial connection, shuts
	 * down background threads and performs clean-up.
	 * <p>
	 * <strong>Notes</strong>
	 * <ul>
	 * <li>Do not connect and disconnect continuously - at least if you can
	 * avoid it. It takes time and resources. Connect once and stay connected.</li>
	 * </ul>
	 * 
	 * @see CService#connect()
	 * 
	 *      AGA Modifications (18/04/2008) Modified method so that it will
	 *      always try to disconnect, even if connected is set false, and if
	 *      receiveThread,keepAliveThread,serialDriver are null, it will just
	 *      ignore them.
	 */
	public void disconnect() throws NotConnectedException {
		if (!connected)
			log.warn("Not connected.  Will attempt disconnection anyway.");
		if (receiveThread == null)
			log.warn("ReceiveThread == null");
		if (keepAliveThread == null)
			log.warn("KeepAliveThread == null");
		if (serialDriver == null)
			log.warn("SerialDriver == null");
		...
		while (timeout > 0
				&& ((receiveThread != null && !receiveThread.isStopped()) || (keepAliveThread != null && !keepAliveThread
						.isStopped()))) {
		...
		if (keepAliveThread != null && !keepAliveThread.isStopped()) {
			...
			// We no longer join() on the keepAliveThread, as this has a
			// tendency to block
	 ...
	 */
	public void readMessages(LinkedList<CIncomingMessage> messageList,
			MessageClass messageClass) throws IOException,
			SMSLibDeviceException {
		switch (protocol) {
		case PDU:
	...
	/**
	 * Gets the next line from this {@link BufferedReader} which contains
	 * information. This either comes in the form of an End-of-Buffer, signified
	 * by <code>null</code>, or a line who's trimmed form has a positive length.
	 * 
	 * @param reader
	 *            Buffered reader from which to read lines.
	...
	static String getNextUsefulLine(BufferedReader reader) throws IOException {
		...
		while ((line = reader.readLine()) != null
			...
			/*
			 * Keep reading until we find a line containing info, or get to the
			 * end of the reader.
			 */
		}
		return line;
	...
	/**
	 * Attempts to create a new message from a PDU, memoryLocation and
	 * memoryIndex. If the message can be created, it is added to the supplied
	 * list of messages.
	 * 
	 * @param messageList
	 *            List to append the new message object to.
	 * @param pdu
	 *            The hex-encoded PDU to decode the message from. This PDU
	 *            should be as-received on a device.
	 * @param memoryLocation
	 * @param memIndex
	 *            The memory index of the PDU, read using
	 *            {@link #getMemIndex(String)}.
	 * @throws MessageDecodeException
	 * @throws NullPointerException
	 *             if the supplied PDU is <code>null</code>.
	 */
	private void createMessage(LinkedList<CIncomingMessage> messageList,
			String pdu, int memoryLocation, int memIndex)
			throws MessageDecodeException {
		if (isIncomingMessage(pdu)) {
			log.info("PDU appears to be an incoming message.  Processing accordingly.");
			CIncomingMessage msg = new CIncomingMessage(pdu, memIndex,
					atHandler.getStorageLocations().substring(
							(memoryLocation * 2), (memoryLocation * 2) + 2));
			// Check if this message is multipart.
			if (!msg.isMultipart()) {
				// This message has only one part, so add it to the list for
				// immediate processing
				log.info("Single part message; adding to message list.");
				messageList.add(msg);
				deviceInfo.getStatistics().incTotalIn();
			} else {
				// This message has multiple parts, so add it to the list of
				// concat messages
			...
			messageList.add(new CStatusReportMessage(pdu, memIndex, atHandler
					.getStorageLocations().substring((memoryLocation * 2),
							(memoryLocation * 2) + 2)));
			deviceInfo.getStatistics().incTotalIn();
		} else {
	...
	private void readMessages_PDU(LinkedList<CIncomingMessage> messageList,
			MessageClass messageClass) throws IOException,
			SMSLibDeviceException {
		synchronized (_SYNC_) {
			if (!isConnected()) {
				...
				log.debug("Checking storage locations: "
						+ atHandler.getStorageLocations());
				for (int ml = 0; ml < (atHandler.getStorageLocations().length() / 2); ml++) {
					String memoryLocation = atHandler.getStorageLocations()
					...
					if (atHandler.setMemoryLocation(memoryLocation)) {
						...
						try {
							reader = new BufferedReader(new StringReader(
									response));
							String line;
							// Read until the end of the response. This is
							// indicated by either a
							// null line, or a line reading "OK".
							while ((line = getNextUsefulLine(reader)) != null
									&& !line.equalsIgnoreCase("OK")) {

								// The first line appears to contain the memory
								// location of this message, so extract it
								...
								if (pdu == null) {
									...
									try {
										createMessage(messageList, pdu, ml,
												memIndex);
									} catch (Throwable t) {
										log.error("Problem processing PDU: "
												+ pdu, t);
									}
								}
							}
	...
	/**
	 * Adds a concatenated message part to the internal list of concatenated
	 * messages. If other parts of this message already exist, we will add this
	 * message to those ones. If this part is deemed a duplicate, it will not
	 * replace the old part we have stored.
	 * 
	 * @param messagePart
	 *            message part loaded from a single PDU. This message part MUST
	 *            NOT be a single-part message.
	 */
	private void addConcatenatedMessagePart(CIncomingMessage messagePart) {
		log.debug("IN-DTLS: MI:" + messagePart.getMemIndex() + " REF:"
				+ messagePart.getMpRefNo() + " MAX:" + messagePart.getMpMaxNo()
				+ " SEQ:" + messagePart.getMpSeqNo());
		// Check to see if this message is part of a multipart message that we
		// already have a
		// bit of. If this is the case, we'll join it up with the other bits. If
		// it is a
		// duplicate then TODO we should probably delete it from the device. If
		// it is not related
		...
		for (int k = 0; k < mpMsgList.size(); k++) {
			...
			// We can tell if a message is part of another message by checking
			// the reference number
			// and the sender number are the same.
			if (listMsg.getMpRefNo() == messagePart.getMpRefNo()
					&& listMsg.getOriginator().equals(
							messagePart.getOriginator())) {
				...
				for (int l = 0; l < tmpList.size(); l++) {
					...
					if (listMsg.getMpSeqNo() == messagePart.getMpSeqNo()) {
						// Our message part is a duplicate. Log this fact, and
						// quit the method.
						log.info("Duplicate message part; ignoring.");
						return;
					}
				}
				// This part was not a duplicate, so add it to the parts list,
				// and stop processing.
				tmpList.add(messagePart);
				return;
			}
		}
		// We didn't find a message that this was a part of. Create a new parts
		// list,
	...
	/**
	 * Gets the memory index of a message from the relevant line of message list
	 * response. TODO show an example line
	 * 
	 * @param responseLine
	 * @return
	 */
	...
	private void readMessages_TEXT(LinkedList<CIncomingMessage> messageList,
			MessageClass messageClass) throws IOException,
			SMSLibDeviceException {
		int i, j, memIndex;
		byte[] bytes;
		...
		synchronized (_SYNC_) {
			if (isConnected()) {
				atHandler.switchToCmdMode();
				for (int ml = 0; ml < (atHandler.getStorageLocations().length() / 2); ml++) {
					if (atHandler.setMemoryLocation(atHandler
						...
						while (true) {
							...
							tokens.nextToken();
							tokens.nextToken();
							if (Character.isDigit(tokens.nextToken().trim()
									.charAt(0))) {
								Calendar cal1 = Calendar
										.getInstance(TIMEZONE_UTC);
								...
								dateStr = tokens.nextToken().replaceAll("\"",
										"");
								cal1.set(Calendar.YEAR, 2000 + Integer
										.parseInt(dateStr.substring(0, 2)));
								cal1.set(Calendar.MONTH, Integer
										.parseInt(dateStr.substring(3, 5)) - 1);
								cal1.set(Calendar.DAY_OF_MONTH, Integer
										.parseInt(dateStr.substring(6, 8)));
								dateStr = tokens.nextToken().replaceAll("\"",
										"");
								cal1.set(Calendar.HOUR_OF_DAY, Integer
										.parseInt(dateStr.substring(0, 2)));
								cal1.set(Calendar.MINUTE, Integer
										.parseInt(dateStr.substring(3, 5)));
								cal1.set(Calendar.SECOND, Integer
										.parseInt(dateStr.substring(6, 8)));
								dateStr = tokens.nextToken().replaceAll("\"",
										"");
								cal2.set(Calendar.YEAR, 2000 + Integer
										.parseInt(dateStr.substring(0, 2)));
								cal2.set(Calendar.MONTH, Integer
										.parseInt(dateStr.substring(3, 5)) - 1);
								cal2.set(Calendar.DAY_OF_MONTH, Integer
										.parseInt(dateStr.substring(6, 8)));
								dateStr = tokens.nextToken().replaceAll("\"",
										"");
								cal2.set(Calendar.HOUR_OF_DAY, Integer
										.parseInt(dateStr.substring(0, 2)));
								cal2.set(Calendar.MINUTE, Integer
										.parseInt(dateStr.substring(3, 5)));
								cal2.set(Calendar.SECOND, Integer
										.parseInt(dateStr.substring(6, 8)));

								msg = new CStatusReportMessage(
										Integer.parseInt(refNo), memIndex,
										atHandler.getStorageLocations()
												.substring((ml * 2),
														(ml * 2) + 2),
								...
								// FIXME according to bjdw,
								// "only need to convert if the modem is in hex mode, not gsm mode."
								// however, we only get here in TEXT mode
								// (rather than PDU mode), so it's hard to say
								...
								if (true) {
									...
									// msgText =
									// GsmAlphabet.bytesToString(HexUtils.decode(msgText));
									// N.B. should confirm that is exactly what
									// it's doing before replacing!
								}

								msg = new CIncomingMessage(
										cal1.getTimeInMillis(), originator,
										msgText, memIndex, atHandler
												.getStorageLocations()
												.substring((ml * 2),
														(ml * 2) + 2));
								log.debug("IN-DTLS: MI:" + msg.getMemIndex());
	...
	/**
	 * Check if we have any completed messages in our list of multipart message
	 * parts. If we <em>do</em> have any completed messages, we should rebuild
	 * them and add them to the supplied message list.
	 * 
	 * @param messageList
	 *            List to insert all received messages into.
	 */
					...
					for (int m = 0; m < tmpList.size(); m++) {
						...
						if (listMsg.getMpSeqNo() == (l + 1)) {
							if (listMsg.getMpSeqNo() == 1) {
								...
								// TODO not quite sure what is happening here,
								// but this doesn't seem the ideal way to
								// build multipart messages...
								// TODO Does this properly handle multipart
								// messages whose parts are received in the
								// wrong order?
								if (mpMsg != null) {
									if (mpMsg.getMessageEncoding() == SmsMessageEncoding.GSM_7BIT
											|| mpMsg.getMessageEncoding() == SmsMessageEncoding.UCS2) {
										mpMsg.setText(mpMsg.getText()
												+ listMsg.getText());
									} else {
										// FIXME this really doesn't look like
										// it concatenates binary messages
										// properly
										byte[] extraBinary = listMsg
												.getBinary();
										byte[] binary = mpMsg.getBinary();
										byte[] newContent = new byte[binary.length
												+ extraBinary.length];
										System.arraycopy(binary, 0, newContent,
												0, binary.length);
										System.arraycopy(extraBinary, 0,
												newContent, binary.length,
												extraBinary.length);
										binary = newContent;
	 ...
	 * <p>
	 * This method actually wraps the message in a list and calls
	 * #sendMessage(List) that does the job.
	 * 
	 * @param message
	 *            The message to be sent.
	 * @throws NotConnectedException
	 *             Either connect() is not called or modem has been
	 *             disconnected.
	 ...
	 * <p>
	 * The function iterates through the supplied list of COutgoingMessage
	 * objects and tries to send them.
	 * <p>
	 * Upon succesful sending, each COutgoingMessage object should have its
	 * RefNo and DispatchDate fields set to specific values. Upon failure, the
	 * RefNo will be set to 0 and DispatchDate set to null.
	 * 
	 * @param messageList
	 *            A list of COutgoingMessage objects presenting messages that
	 *            will be sent out.
	 * @throws NotConnectedException
	 *             Either connect() is not called or modem has been
	 *             disconnected.
	...
	public void sendMessages(List<COutgoingMessage> messageList)
			throws Exception {
		switch (protocol) {
		case PDU:
	...
	private void sendMessages_PDU(List<COutgoingMessage> messageList)
			throws IOException, UnrecognizedHandlerProtocolException,
			NoResponseException, NotConnectedException, DebugException {
		if (isConnected()) {
			keepGsmLinkOpen();
			for (COutgoingMessage message : messageList)
				sendMessage_PDU(message);
		} else
			throw new NotConnectedException();
	}
	...
	void sendMessage_PDU(COutgoingMessage message) throws IOException,
			NoResponseException, UnrecognizedHandlerProtocolException,
			NotConnectedException {
		for (String pdu : message.generatePdus(smscNumber, outMpRefNo)) {
			// We pass the length of the PDU, in octets, to the device. This
			// length does NOT
			// include the PDU's prefix containing the SMSC number, so this
			// length must be
			// calculated and subtracted from the total.
			/**
			 * The length of the PDU in octets, not including the encoded SMSC
			 * Number.
			 */
			int pduLength = pdu.length() >> 1;
			// If there is no SMSC Number set, we will not have appended one to
			// the start of the message.
			if (smscNumber != null) {
				int smscNumberCharacterCount = smscNumber.length();
				if (smscNumberCharacterCount == 0) {
					// The SMSC is empty, so we only need to remove the space
					// for it's length bit
					--pduLength;
				} else {
					// Ignore prefixed plus - this is accounted for in the
					// encoding info
					if (smscNumber.charAt(0) == '+')
						--smscNumberCharacterCount;
					/** Number of octets encoded SMSC number occupies */
					int smscLen = (smscNumberCharacterCount + 1/*
																 * avoid
																 * rounding
																 * error
																 */) >> 1;
					...
					++smscLen;
					// Subtract octet count of the SMSC Number from the pdu's
					// octet count
					pduLength -= smscLen;
				}
			}

			// Get the reference number for the sent message. For a message that
			// is successfully sent, this will
			// be set by the SMSC; for a failed message, this will be some value
			// less than zero. For this reason,
			// we must update the reference number of the message object even if
			// it is < 0
			int refNo;
			synchronized (_SYNC_) {
		...
		// Increment the concat reference number for this service, so that the
		// next message does
		// not have the same reference number as the previous one.
		outMpRefNo = (outMpRefNo + 1) & 0xFFFF;
	}

	private void sendMessages_TEXT(List<COutgoingMessage> messageList)
			throws Exception {
		if (isConnected()) {
			for (COutgoingMessage message : messageList)
				sendMessage_TEXT(message);
	 ...
	 * 
	 * @param hexText
	 *            A StringBuilder. Should be empty, and will be empty on return.
	 * @param message
	 * @return
	 * @throws IOException
	 ...
	 */
	private void sendMessage_TEXT(COutgoingMessage message) throws IOException,
			NoResponseException, UnrecognizedHandlerProtocolException {
		byte[] bytes = GsmAlphabet.stringToBytes(message.getText());
		String hexText = HexUtils.encode(bytes);
		int refNo;
		synchronized (_SYNC_) {
			refNo = atHandler.sendMessage(0, null, message.getRecipient(),
					hexText);
		}
	...
	/**
	 * Delete the message at a specific location in a specific memory of the
	 * device.
	 * 
	 * @param memIndex
	 *            The index into the message memory.
	 * @param memLocation
	 ...
	 */
	protected void deleteMessage(int memIndex, String memLocation)
	 ...
	 * <p>
	 * <strong>Warning</strong>: Do not pass invalid CIncomingMessage objects to
	 * this call - You may corrupt your modem's storage!
	 * <p>
	 * Delete operations are irreversible.
	 * 
	 * @param message
	 *            The CIncomingMessage object previously read with
	 *            readMessages() call.
	 * @throws NotConnectedException
	 *             Either connect() is not called or modem has been
	 *             disconnected.
	 * @see CIncomingMessage
	 */
	public void deleteMessage(CIncomingMessage message)
			throws NotConnectedException, IOException {
		synchronized (_SYNC_) {
			int memIndex = message.getMemIndex();
			String memLocation = message.getMemLocation();
			// If this message has its memIndex set, we delete it directly.
			// Otherwise, it may be
			// a reference to a multipart message. In this case, we must delete
	...
	/**
	 * Reads (or refreshes) all GSM modem information (like manufacturer, signal
	 * level, etc).
	 * <p>
	 * This method is called automatically upon connection. Should you require
	 * fresh info, you should call it yourself when you need it.
	 * 
	 * @throws NotConnectedException
	 *             Either connect() is not called or modem has been
	 *             disconnected.
	...
	private boolean waitForNetworkRegistration() throws IOException,
			GsmNetworkRegistrationException, DebugException {
		while (true) {
			...
			switch (answer) {
			case 0:
				throw new GsmNetworkRegistrationException(
	...
	public String getModel() throws IOException {
		...
		if (response.toUpperCase().contains("MODEL=")) {
			response = response.substring(response.indexOf("MODEL=")
					+ "MODEL=".length());
		}
		return response;
	}
	...
	public boolean getGprsStatus() throws IOException {
		return (atHandler.getGprsStatus()
				.matches("\\s*[\\p{ASCII}]CGATT[\\p{ASCII}]*1\\s*OK\\s*"));
	}
	...
	public String getMsisdn() throws IOException {
		...
		if (splitPoint != 0)
			response = response.substring(splitPoint,
					response.indexOf('"', splitPoint));
	...
	/**
	 * TODO should implement this properly using constants from
	 * {@link TpduUtils}
	 * 
	 * @param pdu
	 * @return
	...
	private class CKeepAliveThread extends Thread {
		...
		public void killMe() {
			stopFlag = true;
			// This is no longer synchronized on _SYNC_, as _SYNC_ is not
			// released by the write method
			// if there is a problem writing to a device (e.g. the device is
				...
				synchronized (_SYNC_) {
					if (isConnected())
						try {
							...
							if (!stopFlag && log != null)
								log.warn("Unexpected exception", ex);
							// Now an exception's been thrown, we should stop
							// this service.
							stopFlag = true;
							try {
								disconnect();
						...
						try {
							...
							for (int i = 0; i < messageList.size(); i++) {
								...
								if (messageHandler == null) {
									...
									if (messageHandler.received(CService.this,
											message)) {
										deleteMessage(message);
									}
								}
							...
							log.error(
									"Unexpected exception on receive thread.",
									ex);
						}
					}
				}
	...
	/**
	 * Holds values representing the message class of the message to be read
	 * from the GSM device.
	 */
	public static enum MessageClass {
		...
		ALL(4, "ALL"),
		/**
		 * Read unread messages. After reading, all returned messages will be
		 * marked as read.
		 */
		UNREAD(0, "REC UNREAD"),
		...
		// > INSTANCE PROPERTIES
		/**
		 * text ID for this {@link MessageClass} when listing messages on a
		 * device in TEXT mode
		 */
		private final String textModeId;
		/**
		 * integer ID for this {@link MessageClass} when listing messages on a
		 * device in PDU mode
		 */
		 ...
		 * 
		 * @param pduModeId
		 *            value for {@link #pduModeId}
		 * @param textModeId
		 *            value for {@link #textModeId}
		 */
		...
		/**
		 * @return the text ID for this {@link MessageClass} when listing
		 *         messages on a device in TEXT mode.
		 */
		...
		/**
		 * @return the integer ID for this {@link MessageClass} when listing
		 *         messages on a device in PDU mode.
		 */
